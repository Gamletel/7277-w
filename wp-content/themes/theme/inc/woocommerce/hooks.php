...
//        add_filter( 'woocommerce_variable_price_html', [$this, 'custom_variable_product_price'], 9999, 2 ); //Меняем формат стоимости вариативного товара
        ...
        add_action('woocommerce_before_main_content', [$this, 'custom_breadcrumbs'], 20); //Кастомные хлебные крошки
...
//        add_filter('woocommerce_currency_symbol', [$this, 'change_existing_currency_symbol'], 10, 2); //Меняем значек рубля на текст
        ...
        remove_action('woocommerce_before_main_content', 'woocommerce_breadcrumb', 20); // Убираем стандартные хлебные крошки

        /*
         * CATEGORY-CARD
         */
        add_action('woocommerce_before_subcategory', [$this, 'add_category_background'], 15); //Добавляем задний фон карточке категории
...
//        add_action('woocommerce_single_product_summary', [$this, 'if_product_not_stock'], 25); // Если цены нет, то выводим информационное поле
        ...
        add_action('woocommerce_single_product_summary', [$this, 'show_custom_title'], 5); //Выводим название
        add_action('woocommerce_single_product_summary', [$this, 'show_additional_text'], 10); //Выводим доп. текст
        add_action('woocommerce_single_product_summary', [$this, 'show_composition'], 10); //Выводим доп. текст
        add_action('woocommerce_single_product_summary', [$this, 'product_info'], 15); //Информация о товаре (описание, атрибуты)
        add_action('woocommerce_after_add_to_cart_button', [$this, 'add_to_favorite_btn'], 20); //Добавляем кнопку добавления в избранное (обычный товар)
        add_action('woocommerce_single_product_summary', [$this, 'show_nutritional_value'], 30); //Добавляем состав продукта (товара)
//        add_action('woocommerce_after_add_to_cart_quantity', [$this, 'add_to_filter_btn'], 10); //Добавляем кнопку фильтра товаров
        add_action('woocommerce_single_product_summary', [$this, 'close_product_main_info'], 70); //Закрытие блока с основной информацией
        /*Описание и характеристики*/
        add_action('woocommerce_after_single_product_summary', [$this, 'product_description'], 5); //Описание товара
        /*Нижняя часть товара*/
...
//        add_filter( 'woocommerce_thankyou_order_received_text', [$this, 'custom_thankyou_text'], 10, 2 ); //Меняем текст Спасибо за заказ
        ...
        * */
        add_action('wp_enqueue_scripts', [$this, 'pp_custom_scripts_enqueue']);
        add_action('wp_ajax_loadmore', [$this, 'pp_loadmore_ajax_handler']); // wp_ajax_{action}
        add_action('wp_ajax_nopriv_loadmore', [$this, 'pp_loadmore_ajax_handler']); // wp_ajax_nopriv_{action}
        remove_action('woocommerce_after_shop_loop', 'woocommerce_pagination', 10);
        add_action('woocommerce_after_shop_loop', [$this, 'pp_woocommerce_products_load_more'], 9);
